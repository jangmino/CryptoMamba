...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="smart",
        choices={"smart", "smart_w_short", "vanilla", "timed_exit", "no_strategy"},
    )
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            if factors is not None:
                timetamps += [
                    float(x)
                    for x in list(batch.get("Timestamp_orig").numpy().reshape(-1))
                ]
            else:
                timetamps += [float(x) for x in list(ts)]

    if factors is not None:
        scale = factors.get(model.y_key).get("max") - factors.get(model.y_key).get(
            "min"
        )
...
if __name__ == "__main__":
    ...
    if args.config == "all":
        config_list = [
            x.replace(".ckpt", "")
            for x in os.listdir(f"{ROOT}/checkpoints/")
            if "_nv.ckpt" in x
        ]
    ...
    for conf, c in zip(config_list, colors):
        ...
        print(
            f"{conf} -- Final balance: {round(balance, 2)}: Num. trades: {len(balance_in_time)}"
        )
        print(
            f"{conf} -- Maximum Draw Down : {round(max_drawdown(balance_in_time) * 100, 2)}"
        )
        ...
        sns.lineplot(
            x=tmp, y=balance_in_time, color=c, zorder=0, linewidth=2.5, label=label
        )
